...
export function ObservanceChart({ data, title = 'Mon suivi' }: ObservanceChartProps) {
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-white rounded-2xl p-4 shadow-lg border border-[#E5E5EA]">
  ...
  return (
    <div className="bg-white rounded-3xl p-6 shadow-sm">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-2xl text-[#1D1D1F] mb-1">{title}</h3>
          <p className="text-sm text-[#86868B]">Évolution de votre traitement</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        >
          <div className="text-3xl text-[#1D1D1F] mb-1">{avgHours.toFixed(1)}h</div>
          <div className="text-sm text-[#86868B]">Moyenne par nuit</div>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-gradient-to-br from-[#34C759]/10 to-[#30D158]/10 rounded-2xl p-4"
        >
          <div className="text-3xl text-[#1D1D1F] mb-1">{compliance.toFixed(0)}%</div>
          <div className="text-sm text-[#86868B]">Nuits observantes</div>
        </motion.div>
      </div>
        ...
        {filteredData.length === 0 ? (
          <div className="h-full flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-[#86868B] text-lg">Aucune donnée disponible pour cette période</p>
              <p className="text-[#86868B] text-sm mt-2">Vos données d'observance apparaîtront ici</p>
            </div>
          </div>
        ) : (
      ...
      <div className="flex items-center justify-center gap-6 mt-4 text-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[#86868B]">≥ 4h (objectif atteint)</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 bg-[#FF9500] rounded-full"></div>
          <span className="text-[#86868B]">&lt; 4h (à améliorer)</span>
        </div>
      </div>
    </div>
  );
